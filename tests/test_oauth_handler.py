...
def mock_installed_app_flow(mocker):
    ...
    _mock_installed_app_flow.from_client_secrets_file().run_local_server.return_value = MockCredentials()


class TestGoogleOauthHandler:
    def test_token_gets_set(self, tmp_path):
        google_oauth_handler = GoogleOauthHandler(Path(tmp_path, "client_secret.json"), Path(tmp_path, "refresh_token"))
    ...
    def test_token_gets_set_using_existing_refresh_token(self, mocker, tmp_path):
        ...
        mock_post = mocker.patch(
            ...
            return_value=MockSuccessResponse(bytes(json.dumps({"access_token": new_access_token}), "utf-8")),
        )

        google_oauth_handler = GoogleOauthHandler(Path(tmp_path, "client_secret.json"), refresh_token_path)
